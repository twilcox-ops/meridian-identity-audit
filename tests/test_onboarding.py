...
def test_rollback_matching_confirmation_proceeds_with_dry_run_false(monkeypatch):
    ...
    monkeypatch.setattr(onboarding, "get_access_token", lambda config: "fake-token")
    monkeypatch.setattr(
        onboarding, "find_run_entries", lambda path, upn, timestamp=None: ("2024-06-01T00:00:00+00:00", [])
    )
    monkeypatch.setattr(onboarding, "run_rollback", fake_run_rollback)
...
def test_rollback_mismatched_confirmation_aborts_before_any_graph_call(monkeypatch):
    monkeypatch.setattr(onboarding, "load_graph_config", _fail_if_called)
    monkeypatch.setattr(onboarding, "get_access_token", _fail_if_called)
    monkeypatch.setattr(onboarding, "find_run_entries", _fail_if_called)
    monkeypatch.setattr(onboarding, "run_rollback", _fail_if_called)
...
def test_rollback_dry_run_never_prompts_for_confirmation(monkeypatch):
    ...
    exit_code = onboarding.main(_ROLLBACK_ARGV, confirm_fn=_fail_if_called)
